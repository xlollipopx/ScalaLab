...
import errors.Errors.AccountValidationError.{AgeIsOutOfBounds, DateIsNotValid, ImpossibleCvv, ImpossiblePassportNumber, InvalidCardNumber, NameIsInvalid}
    ...
    "age" - {
      ...
      "person" - {
        ...
        "invalid person: all fields wrong" in {
          ...
          val rawAge = 0
          val rawPn = "123456"
          val expected = Invalid(Chain(NameIsInvalid, AgeIsOutOfBounds,
            ImpossiblePassportNumber, DateIsNotValid))
      ...
      "payment card" - {
        ...
        "invalid card: all fields are wrong" in {
          ...
          val rawCvv = "22"
          val expected = Invalid(Chain(InvalidCardNumber, DateIsNotValid,
            ImpossibleCvv))
      ...
      "account" - {
        "valid account" in {
          ...
          } yield AccountDto(PersonDto(name, age, pn, birth),
            PaymentCardDto(cardNum, date, cvv))
          assert(actual == expected)
        }
        ...
        "invalid account" in {
          ...
          val expected = Invalid(Chain(NameIsInvalid, AgeIsOutOfBounds,
            ImpossiblePassportNumber, DateIsNotValid, InvalidCardNumber, DateIsNotValid,
            ImpossibleCvv))
